...
const [show, setShow]= useState(false);
  ...
  return (
    <div>
    <div className={`w-[90%] absolute left-0 right-0 z-1000 mx-auto pt-3`}>
      ...
      <div className='md:border-b border-gray-400 dark:border-gray-700 w-full'></div>
        <div className={`dark:border dark:bg-transparent border border-gray-300 dark:border-gray-700 rounded-full flex items-center justify-center gap-3 px-1 py-1 shadow-md`}>
          {navLinks.map((link)=>{
            if (link.type === "modal") {
            return (
            <button key={link.id} onClick={()=> setShow(true)} className='relative transition-all duration-300 hover:text-gray-500 dark:hover:text-white z-10 text-base text-black dark:text-gray-300 py-1 px-3'>{link.label}</button>
          );
          }
          ...
          return (
            <Link
              ...
              {isActive && (
                ...
                layoutId='navIndicator'
                  className='absolute inset-0 rounded-full dark:bg-white bg-neutral-800 shadow-xl'
                  transition={{ type: 'spring', stiffness: 100, damping: 15, bounce: 0.8 }}
                />
              )}
    ...
    <div className='w-fit fixed bottom-4 right-4 md:left-0 md:right-0 z-[10001] md:mx-auto pt-3 pointer-events-auto'>
      <div className={`flex flex-col-reverse md:flex-row justify-end gap-2 cursor-pointer`}>
        {/* Theme Icon */}
