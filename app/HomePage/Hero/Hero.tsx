...
<motion.span
  ...
  FullStack Dev.
    ...
    <Link href='/about' className="relative h-12 w-12 md:h-14 md:w-14 lg:h-12 lg:w-12 overflow-hidden rounded-lg border dark:border-white border-gray-400 hidden md:flex">
      ...
      <p className='text-base'>I build fast, scalable and modern web experiences with clean UI and performance-driven architecture.</p>
      <div className='flex flex-row gap-2'>
        <button onClick={()=>setOpen(true)} className='py-1 px-1 pl-2 border md:text-lg border-gray-400 dark:border-white/70 rounded-full flex items-center gap-2 shadow-xl'>Get in touch<div className='dark:bg-[#e5e4e2] rounded-full w-8 h-8 flex items-center justify-center'><MessagesSquare className='w-5 h-5 text-black animate-bounce mt-1'/></div></button>
